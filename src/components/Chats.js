...
const MessageContainer = styled.div`
  ...
  overflow-y: auto;
  background: #e5ddd6;
  background-image: url("src/image/chat-background.png");
`;
      ...
      <ChatBox>
        <SearchContainer>
          ...
          {pickeVisable && (
            ...
            />)}
          <EmojiImage src="src/image/emoji.svg"
            onClick={() => togglePicker(!pickeVisable)} />
          <SearchInput placeholder='Type a message' onKeyDown={onEnter}
            value={text} onChange={(e) => setText(e.target.value)} />

          <button onClick={updateMssg}><Impjimage src='src/image/enter.png' /></button>
        </SearchContainer>
      </ChatBox>
    </Container>
  )
